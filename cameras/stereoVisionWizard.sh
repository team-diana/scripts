...
select MOD in ${cameraMods[*]}; do
  ...
  startProcessInTerminal "cameras" "roslaunch $launchFile"
  ok "Camera node started"

  if ask "Start stereo_image_proc?"; then
    info "Starting stereo_image_proc..."
    startProcessInTerminal "stereo proc" "ROS_NAMESPACE=stereo rosrun stereo_image_proc stereo_image_proc"

    if ask "Show disparity windows?"; then
      startProcessInTerminal "disparity view" "rosrun image_view disparity_view image:=/stereo/disparity"
    fi
  fi
